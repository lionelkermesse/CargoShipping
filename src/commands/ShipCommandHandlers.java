 ...
 */
public class ShipCommandHandlers{
	private IDomainRepository<Ship> domainRepository;
	
	public ShipCommandHandlers(ShipRepository repository) {
		this.domainRepository = repository;
	}

	public void handles(NotifyShipArrivalCommand command) {
		Ship ship;
		try {
			ship = domainRepository.getById(command.getId());
	...
	public void handles(NotifyShipDepartureCommand command) {
		Ship ship;
		try {
			ship = domainRepository.getById(command.getId());
	...
	public void handles(ChangeShipNameCommand command) {
		Ship ship;
		try {
			ship = domainRepository.getById(command.getId());
			ship.setName(command.getName());
			//repository.save(ship);
		} catch (AggregateNotFoundException e) {
			e.printStackTrace();
		}
